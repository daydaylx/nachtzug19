...
function validateNarrativeVariants(
  ...
  variants.forEach((variant, idx) => {
    // 1. min_drift muss >= 1 sein (keine 0) - nur prüfen wenn definiert
    ...
    // 3. Variant muss min_drift oder condition haben (mindestens eines)
    if (variant.min_drift === undefined && variant.condition === undefined) {
      errors.push({
        type: 'error',
        message: `Szene '${sceneId}' narrative_variant #${idx}: Variante braucht min_drift oder condition`,
        scene_id: sceneId
      });
    }

    // 4. narrative ist nicht leer
